...
env_vars <- c("MTmin.07", "MTmax.01", "northerness", "easterness", "solar_rad", "altitude")
```
...
bk_traits_field_env_m <-
  melt(bk_traits_field_env, measure.vars = c("height_cm", "canopy_area_cm2", "branch_order_num"), variable.name = "Trait", value.name = "Trait.Val")
bk_traits_field_env_m <-
...
bk_traits_lab_species_fieldsite <-
  ...
  summarise(latitude = unique(latitude), longitude = unique(longitude), region = unique(region),
            ...
            northerness = unique(northerness),
            easterness = unique(easterness))
```

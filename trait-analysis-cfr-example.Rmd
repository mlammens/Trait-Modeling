...
```{r}
temp <- indXtraits_cfr_complete_log10[traits]
names(temp) <- c("LMA", "LWR", "Lf_Thick", "Lf_Width", "FWC", "Twig_FWC")
ggpairs(temp)
...
```{r}
indXtraits_cfr_pca
summary(indXtraits_cfr_pca, scaling = 2)
summary(indXtraits_cfr_pca, scaling = 1)
```

```{r}
biplot(indXtraits_cfr_pca, scaling = 2, main = "PCA - scaling = 2", type = c("text", "points"))
biplot(indXtraits_cfr_pca, scaling = 2, choices = c(1,3), main = "PCA - scaling = 2", type = c("text", "points"))
```

```{r}
biplot(indXtraits_cfr_pca, scaling = 1, main = "PCA - scaling = 1", type = c("text", "points"))
```
...
Thus, this can be thought of as a **species-level** trait data set.
...
```{r}
bk_traits_species_pca <- rda(indXtraits_cfr_species[traits], scale = TRUE)
```
...
```{r}
indXtraits_cfr_env <-
  inner_join(indXtraits_cfr_complete_log10, bk_env_fieldsites_complete)
```
...
```{r}
indXtraits_cfr_species_fieldsite <-
  indXtraits_cfr_env %>%
...
```{r}
indXtraits_cfr_species_fieldsite_m <-
  melt(indXtraits_cfr_species_fieldsite, measure.vars = traits, variable.name = "Trait", value.name = "Trait.Val")
indXtraits_cfr_species_fieldsite_m <-
  melt(indXtraits_cfr_species_fieldsite_m, measure.vars = env_vars, variable.name = "Env", value.name = "Env.Val")
```

Make plot

```{r}
ggplot(indXtraits_cfr_species_fieldsite_m, aes(x = Env.Val, y = Trait.Val)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_grid(Trait ~ Env, scales = "free")
```
...
```{r}
spec_both <- intersect( indXtraits_cfr_species$species, names(pXs) )

## Extract only those species with traits from both the pXs and tXs data.frames
pXs_forCWM <- pXs[ spec_both ]
tXs_forCWM <- filter(indXtraits_cfr_species, species %in% spec_both )
...
bk_cwm_traits_pca <- rda( bk_cwm_traits[traits], scale = TRUE )
...
bk_cwm_traits_pca_scores <- scores(bk_cwm_traits_pca, choices = 1:length(traits))$sites
...
bk_cwm_traits_pca_loadings <- 
  scores(bk_cwm_traits_pca, choices = 1:length(traits))$species 
...
```{r}
bk_traits_ind_rda <- 
  rda(formula = indXtraits_cfr_env[traits] ~ MTmax.01 + MTmin.07 + altitude + solar_rad, data = indXtraits_cfr_env)
```
...
```{r}
bk_cwm_traits_rda <- 
  rda(formula = bk_cwm_traits_env[traits] ~ MTmax.01 + MTmin.07 + altitude + solar_rad, data = bk_cwm_traits_env)

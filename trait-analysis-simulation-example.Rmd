...
```{r, eval=FALSE}
summary(indXtraits_sim_pca, scaling = 2)
summary(indXtraits_sim_pca, scaling = 1)
```


```{r}
biplot(indXtraits_sim_pca, scaling = 2, main = "PCA - scaling = 2", type = c("text", "points"))
biplot(indXtraits_sim_pca, scaling = 2, choices = c(1,3), main = "PCA - scaling = 2", type = c("text", "points"))
```

```{r}
biplot(indXtraits_sim_pca, scaling = 1, main = "PCA - scaling = 1", type = c("text", "points"))
```
...
traitsXspeciesXenv <- merge(indXtraits_sim, r_sim, by = "plot")
...
```{r}
traitsXspeciesXenv_std <- traitsXspeciesXenv
traitsXspeciesXenv_std[3:7] <- scale(traitsXspeciesXenv_std[3:7])
```


```{r}
indXtraits_sim_rda <-
  rda(formula = traitsXspeciesXenv[c("lma", "c_to_n", "succulence")] ~ ev1 + ev2, data = traitsXspeciesXenv_std) 
```
...
text(indXtraits_sim_rda,display='species',col='red',axis.bp=TRUE, scaling = 1)
...
text(indXtraits_sim_rda,display='species',col='red',axis.bp=TRUE, scaling = 2)
